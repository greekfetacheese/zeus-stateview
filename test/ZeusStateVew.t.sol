...
contract ZeusStateViewTest is Test {
    ...
    function testgetV3PoolState2() public view {
        ...
        pools[0] = ZeusStateView.V4Pool({pool: V4_ETH_UNI, tickSpacing: 60});

        ZeusStateView.V4PoolData[] memory poolData = zeusStateView.getV4PoolState(pools, STATE_VIEW);
        assertEq(poolData.length, 1);
    }

    function testgetV4PoolState2() public view {
        ZeusStateView.V4Pool[] memory pools = new ZeusStateView.V4Pool[](2);
        pools[0] = ZeusStateView.V4Pool({pool: V4_ETH_UNI, tickSpacing: 60});
        pools[1] = ZeusStateView.V4Pool({pool: V4_LINK_USDC, tickSpacing: 60});
