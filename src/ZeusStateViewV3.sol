...
contract ZeusStateViewV3 {
    ...
    function _getV2Reserves(address[] memory pools) internal view returns (V2PoolReserves[] memory) {
        ...
        for (uint256 i = 0; i < pools.length; i++) {
            // Isolate failures so one bad address does not revert the whole batch.
            try this.fetchV2Reserves(pools[i]) returns (V2PoolReserves memory poolReserves) {
    ...
    /// @dev External so callers can `try/catch` per pool.
    function fetchV2Reserves(address pool) external view returns (V2PoolReserves memory) {
        (uint112 reserve0, uint112 reserve1, uint32 blockTimestampLast) = IUniswapV2Pool(pool).getReserves();
        return
            V2PoolReserves({
                pool: pool,
                reserve0: reserve0,
    ...
    function _getV3PoolState(V3Pool[] memory pools) internal view returns (V3PoolData[] memory) {
        ...
        for (uint256 i = 0; i < pools.length; i++) {
            // Isolate failures so one broken / non-standard pool cannot nuke the batch.
            try this.fetchV3PoolData(pools[i]) returns (V3PoolData memory data) {
                allPoolData[i] = data;
            } catch {}
        }
    ...
    /// @dev External so callers can `try/catch` per pool.
    function fetchV3PoolData(V3Pool memory _pool) external view returns (V3PoolData memory) {
        return _getV3PoolData(_pool);
    }

    function _getV3PoolData(V3Pool memory _pool) internal view returns (V3PoolData memory) {
        IUniswapV3Pool pool = IUniswapV3Pool(_pool.addr);

        (uint160 sqrtPriceX96, int24 tick, , , , , ) = pool.slot0();

        int24 tickSpacing = pool.tickSpacing();
        require(tickSpacing != 0, "V3 tickSpacing=0");
    ...
    function _getV4PoolState(V4Pool[] memory pools, address stateView) internal view returns (V4PoolData[] memory) {
        ...
        for (uint256 i = 0; i < pools.length; i++) {
            // Isolate failures (zero spacing, missing pool, bad StateView, etc.).
            try this.fetchV4PoolData(pools[i], stateView) returns (V4PoolData memory data) {
                allPoolData[i] = data;
            } catch {}
        }
    ...
    /// @dev External so callers can `try/catch` per pool.
    function fetchV4PoolData(V4Pool memory _pool, address stateView) external view returns (V4PoolData memory) {
        return _getV4PoolDataOne(_pool, stateView);
    }

    function _getV4PoolDataOne(V4Pool memory _pool, address stateView) internal view returns (V4PoolData memory) {
        require(_pool.tickSpacing != 0, "V4 tickSpacing=0");

        IStateView stateViewContract = IStateView(stateView);

        (uint160 sqrtPriceX96, int24 tick, , ) = stateViewContract.getSlot0(_pool.pool);
        (int16 wordPos, ) = position(tick / _pool.tickSpacing);

        uint256 tickBitmap = stateViewContract.getTickBitmap(_pool.pool, wordPos);
        uint128 liquidity = stateViewContract.getLiquidity(_pool.pool);

        (uint128 liquidityGross, int128 liquidityNet) = stateViewContract.getTickLiquidity(_pool.pool, tick);

        (uint256 feeGrowthGlobal0, uint256 feeGrowthGlobal1) = stateViewContract.getFeeGrowthGlobals(_pool.pool);
